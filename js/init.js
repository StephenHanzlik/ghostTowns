  ...
  $(document).ready(function() {
    ...
    $xhr0.done(function(data) {
      if ($xhr0.status === 200 || $xhr0.status === undefined) {
        ...
        $temp1.empty();
        $temp1.append(data.data[5]["Observed Air Temperature (degrees farenheit)"] + "°F");
        $snowDepth1.empty();
    ...
    $xhr1.done(function(data) {
      if ($xhr1.status === 200 || $xhr1.status === undefined) {
        ...
        $temp2.empty();
        $temp2.append(data.data[5]["Observed Air Temperature (degrees farenheit)"] + "°F");
        $snowDepth2.empty();
    ...
    $xhr2.done(function(data) {
      if ($xhr2.status === 200 || $xhr2.status === undefined) {
        ...
        $temp3.empty();
        $temp3.append(data.data[5]["Observed Air Temperature (degrees farenheit)"] + "°F");
        $snowDepth3.empty();
    ...
    $xhr3.done(function(data) {
      if ($xhr3.status === 200 || $xhr3.status === undefined) {
        ...
        $temp4.empty();
        $temp4.append(data.data[5]["Observed Air Temperature (degrees farenheit)"] + "°F");
        $snowDepth4.empty();
      ...
      $xhr.done(function(data) {
        if ($xhr.status === 200 || $xhr.status === undefined) {
          ...
          var tempCalc = function() {
            console.log(data.data);
            for (var q = 0; q < timeSearch + 1; q++) {
              dailySnowArr29.push(parseInt(data.data[q]["Observed Air Temperature (degrees farenheit)"]));
              var str = data.data[q].Date;
              sliceYear = str.substring(10, 8);
              if (str.charAt(5) === "0") {
                sliceMonth = str.substring(7, 6);
